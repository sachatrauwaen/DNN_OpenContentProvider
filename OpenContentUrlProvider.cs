...
namespace Satrabel.Modules.OpenContentUrlProvider

{
    public class OpenContentUrlProvider : DotNetNuke.Entities.Urls.ExtensionUrlProvider
    {
                                                 ...
                                                 ref List<string> messages)
        {
            ...
            if (string.IsNullOrEmpty(friendlyUrlPath) == false && Regex.IsMatch(friendlyUrlPath, @"(^|/)(mid|moduleId)/\d+/?", RegexOptions.IgnoreCase) == false)
            {
                ...
                Regex groupUrlRegex = new Regex(@"(?<l>/)?id/(?<id>\d+)", RegexOptions.IgnoreCase);
                ...
                if (groupUrlMatch.Success)
                {
                    ...
                    string rawId = groupUrlMatch.Groups["id"].Value;
                    int id = 0;
                    if (int.TryParse(rawId, out id))
                    {
                        ...
                        var rules = Satrabel.OpenContent.Components.UrlRewriter.OpenContentUrlProvider.GetRules(tab.PortalID)
                                    .Where(r => r.TabId == tab.TabID);
                        ...
                        var rule = rules.FirstOrDefault(r => r.Parameters == "id=" + id && r.CultureCode == cultureCode);
                        if (rule == null)
                        {
                            rule = rules.FirstOrDefault(r => r.Parameters == "id=" + id && r.CultureCode == null);
                        }
                        if (rule != null)
                        {
                            path = rule.Url;
                                                                 ...
                                                                 PortalAliasInfo portalAlias, ref List<string> messages, out int status, out string location)
        {
            ...
            if (string.IsNullOrEmpty(path) == false && Regex.IsMatch(path, @"(^|/)(mid|moduleId)/\d+/?", RegexOptions.IgnoreCase) == false)
            {
                if (urlParms.Length > 0)
                {
                    ...
                    var rules = Satrabel.OpenContent.Components.UrlRewriter.OpenContentUrlProvider.GetRules(portalId)
                    ...
                    for (int i = urlParms.GetUpperBound(0); i >= 0; i--)
                    {
                        ...
                        var rule = rules.FirstOrDefault(r => r.Url == qsKey && r.CultureCode == cultureCode );
                        if (rule == null)
                        {
                            rule = rules.FirstOrDefault(r => r.Url == qsKey && r.CultureCode == null);
                        }
                        if (rule != null)
                        {
